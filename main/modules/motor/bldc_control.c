...
{
    ESP_LOGI(TAG,
             "Init BLDC wheel %d PWM GPIO=%d channel=%d",
             wheel_index + 1,
             config->pwm_gpio,
             config->pwm_channel);
...
{
    ESP_LOGI(TAG,
             "Init BLDC wheel %d encoder A=%d B=%d",
             wheel_index + 1,
             config->enc_gpio_a,
             config->enc_gpio_b);
    ...
    ESP_RETURN_ON_ERROR(pcnt_unit_set_glitch_filter(pcnt_unit, &filter_config), TAG, "pcnt_unit_set_glitch_filter failed");
    ...
    ESP_RETURN_ON_ERROR(pcnt_channel_set_edge_action(pcnt_chan_a,
                                                     PCNT_CHANNEL_EDGE_ACTION_DECREASE,
                                                     PCNT_CHANNEL_EDGE_ACTION_INCREASE),
                        TAG,
                        "pcnt_channel_set_edge_action A failed");
    ESP_RETURN_ON_ERROR(pcnt_channel_set_level_action(pcnt_chan_a,
                                                      PCNT_CHANNEL_LEVEL_ACTION_KEEP,
                                                      PCNT_CHANNEL_LEVEL_ACTION_INVERSE),
                        TAG,
                        "pcnt_channel_set_level_action A failed");
    ESP_RETURN_ON_ERROR(pcnt_channel_set_edge_action(pcnt_chan_b,
                                                     PCNT_CHANNEL_EDGE_ACTION_INCREASE,
                                                     PCNT_CHANNEL_EDGE_ACTION_DECREASE),
                        TAG,
                        "pcnt_channel_set_edge_action B failed");
    ESP_RETURN_ON_ERROR(pcnt_channel_set_level_action(pcnt_chan_b,
                                                      PCNT_CHANNEL_LEVEL_ACTION_KEEP,
                                                      PCNT_CHANNEL_LEVEL_ACTION_INVERSE),
                        TAG,
                        "pcnt_channel_set_level_action B failed");
...
esp_err_t bldc_control_init(bldc_control_system_t *system)
{
    ...
    for (int i = 0; i < NUM_MOTORS; i++)
    {
        ...
        ESP_RETURN_ON_ERROR(bldc_encoder_init(i, &encoder_configs[i], &system->encoders[i]),
                            TAG,
...
esp_err_t bldc_control_set_drive(bldc_control_system_t *system, int16_t left, int16_t right)
{
    ...
    // ESP_RETURN_ON_ERROR(bldc_control_set_pwm_percent(system, 0, left_percent), TAG, "set motor 1 PWM failed");
    // ESP_RETURN_ON_ERROR(bldc_control_set_pwm_percent(system, 1, right_percent), TAG, "set motor 2 PWM failed");
    // ESP_RETURN_ON_ERROR(bldc_control_set_pwm_percent(system, 2, left_percent), TAG, "set motor 3 PWM failed");
    // ESP_RETURN_ON_ERROR(bldc_control_set_pwm_percent(system, 3, right_percent), TAG, "set motor 4 PWM failed");
...
esp_err_t bldc_control_update(bldc_control_system_t *system)
{
    ...
    for (int i = 0; i < NUM_MOTORS; i++)
    {
        ...
        system->snapshot.velocity_rps[i] = (elapsed_s > 0.0f)
                                               ? ((float)delta_count / WHEEL_ENCODER_COUNTS_PER_REV) / elapsed_s
                                               : 0.0f;

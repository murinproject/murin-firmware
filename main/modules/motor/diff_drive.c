...
{
    return isfinite(left) && isfinite(right) && fabsf(left) <= DIFF_DRIVE_MAX_SPEED_MPS && fabsf(right) <= DIFF_DRIVE_MAX_SPEED_MPS;
}
...
void motor_init(void)
{
    if (initialized)
        return;
    const ledc_timer_config_t tc = {.speed_mode = MOTOR_PWM_MODE, .duty_resolution = MOTOR_PWM_RESOLUTION, .timer_num = MOTOR_PWM_TIMER, .freq_hz = BLDC_PWM_FREQ_HZ, .clk_cfg = LEDC_AUTO_CLK};
    CHECK(ledc_timer_config(&tc), "timer config");
    for (size_t i = 0; i < sizeof(pwm_outputs) / sizeof(pwm_outputs[0]); ++i)
    {
        const ledc_channel_config_t cc = {.gpio_num = pwm_outputs[i].gpio, .speed_mode = MOTOR_PWM_MODE, .channel = pwm_outputs[i].channel, .intr_type = LEDC_INTR_DISABLE, .timer_sel = MOTOR_PWM_TIMER, .duty = 0, .hpoint = 0, .flags.output_invert = 0};
        CHECK(ledc_channel_config(&cc), "channel config");
    }
    const gpio_config_t gc = {.pin_bit_mask = (1ULL << MOT_DIR_1) | (1ULL << MOT_DIR_2) | (1ULL << MOT_BRAKE_1) | (1ULL << MOT_BRAKE_2), .mode = GPIO_MODE_OUTPUT, .pull_up_en = GPIO_PULLUP_DISABLE, .pull_down_en = GPIO_PULLDOWN_DISABLE, .intr_type = GPIO_INTR_DISABLE};
    ...
    initialized = true;
    command_timeout_timer = xTimerCreate("motor_timeout", pdMS_TO_TICKS(DIFF_DRIVE_COMMAND_TIMEOUT_MS), pdFALSE, NULL, motor_timeout_callback);

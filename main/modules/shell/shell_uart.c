...
{
    uart_write_bytes(SHELL_UART_PORT, text, strlen(text));
}
...
static void shell_replace_line(char *line, size_t *line_length,
...
static size_t shell_history_load(char history[SHELL_HISTORY_SIZE][SHELL_LINE_MAX_LENGTH])
{
    ...
    if (flash_storage_get(FLASH_STORAGE_ITEM_SHELL_HISTORY, &stored, &stored_size) != ESP_OK ||
        stored_size != sizeof(stored) || stored.magic != SHELL_HISTORY_MAGIC ||
        stored.count > SHELL_HISTORY_SIZE)
        return 0;
...
static void shell_history_save(char history[SHELL_HISTORY_SIZE][SHELL_LINE_MAX_LENGTH],
...
static void shell_task(void *arg)
{
    ...
    while (true)
    {
        ...
        // Handle the ANSI sequence emitted by arrow keys (for example, Up is ESC [ A).
        if (escape_state == 1)
        {
            escape_state = ch == '[' ? 2 : 0;
            continue;
        ...
        if (escape_state == 2)
        {
            ...
            if (ch == 'A' && history_count > 0)
            {
                ...
                shell_replace_line(line, &line_length,
                                   history[history_index],
                                   strlen(history[history_index]));
        ...
        if (escape_state == 3)
        {
            ...
            {
                history_index = -1;
                memmove(&line[cursor_pos], &line[cursor_pos + 1],
                        line_length - cursor_pos - 1);
                line_length--;

                uart_write_bytes(SHELL_UART_PORT, &line[cursor_pos],
        ...
        if (ch == '\r' || ch == '\n')
        {
            ...
            if (line_length > 0)
            {
                size_t entries_to_move = history_count < SHELL_HISTORY_SIZE
                                             ? history_count
                                             : SHELL_HISTORY_SIZE - 1;
                if (entries_to_move > 0)
                    memmove(history[1], history[0],
                            entries_to_move * sizeof(history[0]));
...
void shell_uart_init(void)
{
    ...
    ESP_LOGI(TAG, "Initializing UART shell port=%d tx=%d rx=%d baud=%d",
             SHELL_UART_PORT, SHELL_UART_TX_GPIO, SHELL_UART_RX_GPIO, SHELL_UART_BAUD_RATE);
    ...
    ESP_ERROR_CHECK(uart_set_pin(SHELL_UART_PORT, SHELL_UART_TX_GPIO, SHELL_UART_RX_GPIO,

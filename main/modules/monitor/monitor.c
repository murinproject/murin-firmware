...
static void ros2_monitor_callback(uint8_t msg_type, uint8_t seq,
                                  const uint8_t *payload, size_t payload_len)
{
    ...
    monitor_event_t event = {
        ...
        .data.ros2 = {
            ...
            .msg_type = msg_type,
            .seq = seq,
            .payload_len = (uint16_t)(payload_len > FRAMED_LINK_MAX_PAYLOAD_LEN
                                          ? FRAMED_LINK_MAX_PAYLOAD_LEN
                                          : payload_len),
        },
    };
...
static void append_bar(char *line, size_t line_size, size_t *length, uint16_t value)
{
    ...
    const unsigned filled = ((unsigned)(clamped - RP3_CHANNEL_VALUE_MIN) * 4U) /
                            (RP3_CHANNEL_VALUE_MAX - RP3_CHANNEL_VALUE_MIN);
    if (*length >= line_size)
        return;
    ...
    while (1)
    {
        ...
        if (event.type == MONITOR_EVENT_RP3)
        {
            ...
            size_t length = (size_t)snprintf(line, sizeof(line),
                                             ...
                                             (unsigned)event.data.rp3.uplink_link_quality,
                                             event.data.rp3.uplink_rssi_dbm,
                                             event.data.rp3.uplink_snr_db,
                                             (unsigned)event.data.rp3.downlink_link_quality,
                                             event.data.rp3.downlink_rssi_dbm,
                                             event.data.rp3.downlink_snr_db);
            s_rp3_display_started = true;
            for (size_t i = 0; i < 8; i++)
            {
                append_bar(line, sizeof(line), &length,
                           event.data.rp3.rc_channels_valid
                               ? event.data.rp3.rc_channels[i]
                               : 0);
            }
        ...
        if (event.type == MONITOR_EVENT_DIFF_DRIVE)
        {
            ...
            shell_printf("[DIFF] PWM=%3u %3u %3u %3u  BRAKE=%1u %1u  DIR=%1u %1u\r\n",
                         ...
                         (unsigned)state->pwm_percent[1],
                         (unsigned)state->pwm_percent[2],
                         (unsigned)state->pwm_percent[3],
                         state->brake[0] ? 1U : 0U,
                         state->brake[1] ? 1U : 0U,
                         state->direction[0] ? 1U : 0U,
        ...
        int length = snprintf(line, sizeof(line),
                              "[ROS2] %" PRId64 " us type=0x%02X seq=%u len=%u payload=",
...
void monitor_init(void)
{
    ...
    if (xTaskCreate(ros2_monitor_task, "monitor", ROS2_MONITOR_TASK_STACK_SIZE,
                    NULL, ROS2_MONITOR_TASK_PRIORITY, NULL) != pdPASS)
    {
...
{
    return s_diff_drive_monitor_enabled;
}

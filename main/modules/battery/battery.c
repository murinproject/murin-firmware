...
static esp_err_t i2c_read_register_16(uint8_t reg_addr, uint16_t *data)
{
  ...
  esp_err_t ret = i2c_master_transmit_receive(s_battery_wattmeter_dev, &reg_addr, sizeof(reg_addr), reg_data,
                                              sizeof(reg_data), BATTERY_I2C_TIMEOUT_MS);
  if (ret != ESP_OK) {
...
static esp_err_t i2c_write_register_16(uint8_t reg_addr, uint16_t value)
{
  ...
  return i2c_master_transmit(s_battery_wattmeter_dev, reg_data, sizeof(reg_data), BATTERY_I2C_TIMEOUT_MS);
}
 ...
 */
static float convert_current(int16_t raw_register) { return (float)raw_register * 0.001f; }
 ...
 */
static float convert_power(uint16_t raw_register) { return (float)raw_register * 0.020f; }
...
void battery_init(void)
{
  ...
  ESP_LOGI(TAG, "Initializing I2C for battery monitoring (SDA=%d, SCL=%d)", BATTERY_I2C_MASTER_SDA_IO,
           BATTERY_I2C_MASTER_SCL_IO);
  ...
  if (ret != ESP_OK) {
    ESP_LOGW(TAG,
             "Wattmeter not found at I2C address 0x%02X, battery telemetry "
             "disabled: %s",
  ...
  if (ret != ESP_OK) {
    ESP_LOGW(TAG, "INA219 configuration failed, battery telemetry disabled: %s", esp_err_to_name(ret));
    battery_cleanup();
    s_battery_initialized = true;
    return;
  }

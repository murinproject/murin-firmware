...
bool flash_storage_get_telemetry_enabled(bool default_enabled)
{
    ...
    return flash_storage_get(FLASH_STORAGE_ITEM_TELEMETRY_ENABLED, &enabled, &size) == ESP_OK
               ? enabled != 0
               : default_enabled;
}
...
uint64_t flash_storage_get_total_runtime_ms(void)
{
    ...
    return flash_storage_get(FLASH_STORAGE_ITEM_TOTAL_RUNTIME_MS, &runtime_ms, &size) == ESP_OK
               ? runtime_ms
               : 0;
}
